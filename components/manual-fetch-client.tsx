...
export default function ManualFetchClient() {
  ...
  const handleStart = async () => {
    if (!date) {
      setMessages(prev => [...prev, { type: 'error', message: 'Please pick a date' }]);
      return;
    }
    ...
    if (!resp.ok) {
      setMessages(prev => [...prev, { type: 'error', message: json?.message || 'Failed to start fetch' }]);
      return;
    }

    const jobId = json.jobId;
    setIsRunning(true);
    setMessages(prev => [...prev, { type: 'queued', message: 'Job queued successfully', jobId }]);
    ...
    es.onmessage = (ev) => {
      try {
        ...
        setMessages(prev => [...prev, { type: 'message', raw: ev.data }]);
      }
    };

    es.onerror = (err) => {
      setMessages(prev => [...prev, { type: 'sse-error', message: 'Connection lost' }]);
      setIsRunning(false);
      try { es.close(); } catch (e) {}
    };
  };
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-700">
        <div className="flex items-center gap-3">
          <div className="p-2 rounded-lg bg-gradient-to-br from-[#29a8e0] to-[#1d8bc4] shadow-lg shadow-[#29a8e0]/20">
            <Database className="w-5 h-5 text-white" />
          </div>
          <div>
            <h3 className="font-semibold text-slate-900 dark:text-slate-100">Manual Data Fetch</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400">Fetch attendance data from source</p>
          </div>
        </div>
        <Button
          variant="ghost"
            ...
            <div className="py-4 space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-[#29a8e0] focus:border-transparent"
                />
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    variant="link"
                    size="sm"
                    className="text-xs h-auto p-0 text-[#29a8e0] hover:text-[#1d8bc4]"
                ...
                <div className="grid grid-cols-2 gap-2 max-h-32 overflow-y-auto p-2 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700">
                  {branches.map((branch) => (
                    <label
                      key={branch}
                    ...
                    >
                      <Checkbox
                        ...
                        className="border-slate-300 dark:border-slate-600 data-[state=checked]:bg-[#29a8e0] data-[state=checked]:border-[#29a8e0]"
                      />
              ...
              <div className="flex gap-2 pt-2">
                <Button
                  onClick={handleStart}
                  disabled={isRunning}
                  className="flex-1 bg-gradient-to-r from-[#29a8e0] to-[#1d8bc4] hover:from-[#1d8bc4] hover:to-[#156a9c] text-white shadow-lg shadow-[#29a8e0]/25 hover:shadow-[#29a8e0]/40 transition-all"
                >
                  {isRunning ? (
                    <>
      ...
      <div className="flex-1 mt-4 min-h-0">
        ...
        <div
          ref={terminalRef}
          className="h-[280px] overflow-y-auto bg-[#0d1117] dark:bg-[#0d1117] rounded-lg border border-slate-800 p-3 font-mono text-xs"
        >
          {messages.length === 0 ? (
            <div className="text-slate-500 italic">Ready to fetch data...</div>
          ) : (
            messages.map((m, i) => (
              <div key={i} className="flex gap-2 py-0.5">
                ...
                <span className={getStatusColor(m.type)}>
                  {m.message || m.raw || JSON.stringify(m, null, 0)}
                </span>
              </div>
            ))
          )}
