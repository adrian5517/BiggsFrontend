...
//   7. Table Components
//   8. Page Component (pure composition)
// ─────────────────────────────────────────────────────────────────────────────
...
export type FilterTab = "all" | "pending" | "approved" | "rejected" | "rescheduled";
...
/** Single source of truth for status labels and Tailwind classes */
...
const FILTER_TABS: FilterTab[] = ["all", "pending", "approved", "rejected", "rescheduled"];
...
/**
 * Centralised status group predicates.
 * Add new statuses here — never duplicate this logic across the file.
 */
const statusIs = {
...
// ─────────────────────────────────────────────────────────────────────────────
// 3. API Layer — all fetch calls are isolated here; nothing else calls fetch()
// ─────────────────────────────────────────────────────────────────────────────
...
/** Returns the branch IDs this manager is authorised to manage */
...
/** Load every booking across all branches assigned to this manager */
...
function useManagerBookings() {
  ...
  /**
   * Immediately reflect a status change in the UI.
   * Callers should trigger a full refresh if the server returns an error.
   */
  ...
  return (
    <div className="rounded-2xl border border-blue-100 bg-blue-50/60 p-4">
      ...
      {booking.promo_description && (
        <p className="text-gray-600 text-xs leading-relaxed mb-3">{booking.promo_description}</p>
      )}
    ...
    <div className="grid grid-cols-2 gap-2.5">
      {items.map(({ label, value, mono, icon }) => (
        <div key={label} className="bg-gray-50 rounded-2xl p-3.5 border border-gray-100">
          ...
          <p className={`text-gray-800 font-semibold text-sm truncate ${mono ? "font-mono" : ""}`}>
            {value}
          </p>
        </div>
      ))}
...
/** Form state is fully owned here — never hoisted to the page */
function PendingActions({
  ...
  const canReschedule = reschedDate.length > 0 && reschedTime.length > 0 && reschedReason.trim().length > 0;

  // Non-pending bookings only get a close button
      ...
      >
        ...
        <div className="px-6 pt-5 pb-6 space-y-5">
          ...
          <PendingActions
            ...
            error={actionError}
            onApprove={() => onStatusUpdate(booking.id, { status: "approved" })}
            onDecline={(reason) => onStatusUpdate(booking.id, { status: "declined", reason })}
            onReschedule={(p) => onStatusUpdate(booking.id, { status: "rescheduled", ...p })}
            onClose={onClose}
          />
        </div>
      </div>
    ...
    <div className="bg-white rounded-3xl border border-gray-200 shadow-sm overflow-hidden">
      <div className="flex items-center justify-between px-7 py-4 border-b border-gray-100">
        <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">
          {bookings.length} {filterStatus === "all" ? "total" : filterStatus} booking{bookings.length !== 1 ? "s" : ""}
        </span>
        {loading && (
        ...
        <table className="min-w-full">
          ...
          <tbody>
            {bookings.map((booking, i) => (
              <BookingRow key={booking.id} booking={booking} index={i} onView={onView} />
            ))}
          </tbody>
        </table>
      </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// 8. Page Component — pure composition, no fetch logic
// ─────────────────────────────────────────────────────────────────────────────
...
const ManagerBookingsPage = () => {
  ...
  // Load manager branch IDs once on mount for client-side auth guard
  useEffect(() => {
    fetchManagerBranchIds()
      .then(setManagerBranchIds)
      .catch(() => setManagerBranchIds([]));
  }, []);
  ...
  function openModal(booking: Booking) {
    // Client-side guard: only allow if manager owns this booking's branch
    if (
      typeof booking.branch_id === "number" &&
      managerBranchIds.length > 0 &&
      !managerBranchIds.includes(booking.branch_id)
    ) {
      setActionError("You are not authorised to action bookings from this branch.");
      return;
    }
    ...
    <>
      ...
      <div className="min-h-screen" style={{ backgroundColor: "#f8faff" }}>
        <div className="max-w-7xl mx-auto px-6 py-10">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
            <div>
              ...
              <p className="text-gray-500 mt-2 text-sm">
                All reservations across your assigned branches, updated in real-time.
              </p>
            </div>
          ...
          {/* Page-level error (e.g. branch auth guard, shown outside modal) */}
          ...
          {loading && bookings.length === 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 gap-3 text-gray-400">
              <p className="text-sm">
                No <span className="font-semibold">{filterStatus}</span> bookings.
              </p>
            </div>
          ) : (
            <BookingsTable
